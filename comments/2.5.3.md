...
[note:For user interface components with multiple labels such as cards or tiles, the primary label (typically the heading) must be contained in the accessible name. Secondary labels (such as a subtitle) may be included in the accessible name or must otherwise be exposed through other accessibility properties, ensuring all information remains accessible to assistive technologies.]

[note:Nested controls are separate user interface components; their labels may be exposed as custom accessibility actions or as separate focusable elements.]
