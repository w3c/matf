...
[note:This requirement applies to [application software](#application-software), such as mobile applications that interpret pointer actions (i.e., this does not apply to actions that are required to operate the [platform software](#platform-software), such as the operating system, or assistive technology). Each layer is responsible for its own pointer actions only, not for those in an underlying layer.]
